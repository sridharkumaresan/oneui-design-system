...
- Branded gradients and solids must flow through semantic surface roles exported from `@functions-oneui/theme`.
...
- Approved branded surfaces should reference semantic surface keys such as `heroPrimary` or `panelSpotlight`.
- If a new component needs a branded surface variant, extend the design contract first instead of embedding gradient literals in the component.
